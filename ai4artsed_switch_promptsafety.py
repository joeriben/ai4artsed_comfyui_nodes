...
class ai4artsed_switch_promptsafety:
    """
    Eine intelligente Node, die einen Prompt basierend auf einem wählbaren 
    Sicherheitslevel ("off", "youth", "kids") filtert.
    """

    MANIPULATION_INSTRUCTIONS = {
        "youth": """Ensure that the image remains emotionally and psychologically appropriate for children, without eliminating tension or imaginative depth. If the prompt includes themes of darkness, threat, or unsettling presence, do not suppress or ignore them — instead, modulate their expression through symbolic, restrained, and culturally open visual forms.
...
DO NOT USE ANY META-TERMS, JUST THE INSTRUCTIONS FOR IMAGE GENERATION! Do not explain your reasoning."""
    ...
    def get_combined_model_list(cls):
        # --- OpenRouter-Modelle dynamisch laden, wie im Original ---
        model_info = {
        ...
        # Sicherstellen, dass das bevorzugte Modell an erster Stelle steht
    ...
    RETURN_NAMES = ("filtered_prompt", "filtered_negative_prompt",)
    FUNCTION = "execute"
    CATEGORY = "AI4ArtsEd"

    def execute(self, prompt, filter_level, model, api_key, negative_prompt=""):
        if filter_level == "off":
        ...
        full_llm_prompt = (
            f"Task:\n{self.GENERAL_TRANSFORMATION_PROMPT.strip()}\n\n"
            f"Context:\n{instruction.strip()}\n\n"
            f"Prompt:\n{prompt.strip()}"
        )
        ...
        try:
            if model.startswith("openrouter/"):
                ...
                filtered_text = self.call_ollama(full_llm_prompt, real_model_name)
            else:
